...
Lambda 関数（動画変換）のデプロイは [Lambda デプロイガイド](./lambda-deploy-guide.md) を参照してください。
...
2. **Zone Resources**: `All zones` または `screenbase.dpdns.org` のゾーンのみ
...
# AWS Lambda（動画変換）
gh secret set LAMBDA_FUNCTION_URL
...
| `LAMBDA_FUNCTION_URL` | Lambda Function URL | 動画変換 Lambda の呼び出し先 |
...
Secrets は 9 件、Variables は 5 件が表示されれば OK です。
...
4. **Queue** — 作成（初回）、動画変換ジョブのキュー
5. **Web (Next.js)** — OpenNext でビルド → Workers にデプロイ + `screenbase.dpdns.org` に紐付け
6. **Server (Hono)** — tsdown でビルド → Workers にデプロイ + `api.screenbase.dpdns.org` に紐付け
...
2. `LAMBDA_FUNCTION_URL`, `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY` が GitHub Secrets に正しく設定されているか確認
3. Lambda のログを確認: `aws logs tail /aws/lambda/screenbase-video-processor --follow`
...
- [ ] GitHub Secrets に 9 つの値をすべて登録した（Step 3）
